...
export class Contract {
  ...
  collectInkImports() {
    if (this.inkImports.length === 0 && (!this.extensions || this.extensions.filter((e) => e.inkImports && e.inkImports.length).length === 0))
      return ''
  ...
  collectStorageFields() {
    return `${this.storage ? this.storage.toString() : ''}${
      this.storage && this.extensions && this.extensions.filter((e) => e.storage).length ? '\n' : ''
    }${
      this.extensions
        ? this.extensions
            .filter((e) => e.storage)
  ...
  collectConstructorArgs() {
    return `${this.constructorArgs.join(', ')}${
      this.constructorArgs?.length && this.extensions.filter((e) => e.constructorArgs?.length).length ? ', ' : ''
    }${this.extensions
  ...
  collectConstructorActions() {
    return `${
      ...
      this.extensions && this.extensions.filter((e) => e.constructorActions?.length).length
        ? `\n${this.extensions
            .filter((e) => e.constructorActions?.length)
            .map(
              (e) => `${e.constructorActions.map((a) => `${this.version === 'v1.3.0' || this.version > 'v2.3.0' ? '' : '\t'}\t\t\t${a}`).join('\n')}`
            )
            .join('\n')}`
        : ''
    }`
        ...
        pub fn new(${this.collectConstructorArgs()}) -> Self {
            ${
              this.version === 'v1.3.0' || this.version > 'v2.3.0'
                ...
                : `ink${this.version < 'v3.0.0-beta' ? '_lang' : ''}::codegen::initialize_contract(|_instance: &mut Contract|{`
            }${this.collectConstructorActions()}${this.version > 'v1.3.0' && this.version < 'v3.0.0-beta' ? '\n\t\t\t})' : '\n\t\t\t_instance'}
...
export class Method {
  ...
  toString() {
    ...
    if (this.args.length < 2) {
      result += `\t\t${this.isPublic ? 'pub ' : ''}fn ${this.name}(&${this.mutating ? 'mut ' : ''}self${this.args.length ? ', ' : ''}${this.args
        .map((a) => a.toString())
